...
export default function DebugPanel({ info }: DebugPanelProps) {
  ...
  return (
    <div className="debug-panel">
      ...
      <div className="debug-row">
        <span>Mode</span>
        <strong>{performance.enabled ? "ON" : "OFF"}</strong>
      </div>
      ...
      {performance.enabled && (
        <>
          <div className="debug-row">
            <span>Grid Size</span>
            <strong>{performance.gridSize}</strong>
          </div>

          <div className="debug-row">
            <span>Total Cells</span>
            <strong>{performance.totalCellCount.toLocaleString()}</strong>
          </div>

          <div className="debug-row">
            <span>Configured Fail Rate</span>
            <strong>{performance.configuredFailRate.toFixed(1)} %</strong>
          </div>

          <div className="debug-row">
            <span>Actual Fail Rate</span>
            <strong>{performance.failRate.toFixed(1)} %</strong>
          </div>

          <div className="debug-row">
            <span>Fail Cells</span>
            <strong>{performance.failCount.toLocaleString()}</strong>
          </div>

          <div className="debug-row">
            <span>Data Generation</span>
            <strong>{performance.dataGenerationMs.toFixed(1)} ms</strong>
          </div>

          <div className="debug-row">
            <span>Thin Instance Build</span>
            <strong>{performance.thinInstanceBuildMs.toFixed(1)} ms</strong>
          </div>

          <div className="debug-row">
            <span>Load to First Frame</span>
            <strong>{performance.loadToFirstFrameMs.toFixed(1)} ms</strong>
          </div>
        </>
      )}

      <div className="debug-section">Rendering</div>
